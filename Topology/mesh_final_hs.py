...
class MeshProtocol(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        if self.name is None:
            self.name = message
            for client in self.factory.clients: #self.factory.clients is list here.
                if client!=self:
                    client.transport.write(f"{self.name} joined the chat.\n".encode())
...
if __name__ == "__main__":
    reactor.listenTCP(8702, MeshFactory())
