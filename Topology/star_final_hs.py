...
class StarProtocol(protocol.Protocol):
    ...
    def connectionMade(self):
        ...
        print('New client connected: ',self.transport.getPeer())#IPv4 or IPv6
    ...
    def dataReceived(self, data):
        ...
        if not self.name:
            self.name = message                                #here msg is clients name
            print(self.name,' has connected to the server.')   #if the client has not connected to the server before 

        else:
            if message.startswith('@'):     #private msg                   #already existing client sends a message 
                '''one client will send message to another client '''
                recipient, private_message = message[1:].split(":", 1)  #one occurance
